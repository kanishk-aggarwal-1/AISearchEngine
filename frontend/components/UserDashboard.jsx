...
export default function UserDashboard({
  ...
  savedSessions, sessionLabel, setSessionLabel,
  onSaveSession, result,
  followed, alerts, bookmarks,
}) {
      ...
      <article className="query-card">
        ...
        <div className="inline-row">
          <input
            ...
            placeholder="Label current search session"
          />
          <button
            type="button"
            onClick={() => onSaveSession(result?.context_id, session?.token)}
            disabled={!result?.context_id}
          >
            Save current
          </button>
        </div>
            ...
            savedSessions.map((item) => (
              <div key={item.id} className="bookmark-item">
                <div>
                  ...
                  <p className="muted">
                    {item.context_id} | {new Date(item.created_at).toLocaleString()}
                  </p>
                </div>
              </div>
            ))
